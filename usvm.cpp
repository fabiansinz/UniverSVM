	...
	for(i=1;i<argc;i++)
	{
		...
		switch(argv[i-1][1])
		{
			...
			case 'S':
			    ...
			    break;
		        case 'V':
			    if (atoi(argv[i]) != 0) use_universum = atoi(argv[i]);
			    break;
		        case 'B':
...
int split_file_load(char *f)
{
	...
	while(1)
	{
		...
		if (labs) ysplits.push_back(j);
	}
	printf("split size = %d  %d\n",splits.size(),ysplits.size());
	return binary_file;
}
...
// loads the same format as LIBSVM
{
	...
	{
		case '\n':
			if(m==0 && splits.size()>0) // training set split
			{
				if(splitpos<splits.size() && splits[splitpos]==msz)
				{
	...
	rewind(fp);
     ...
     for(i=0;i<msz;i++)
     {
		...
		if(splits.size()>0)
		{
			if(splitpos<splits.size() && splits[splitpos]==i)
			{
				write=2;splitpos++;
			}
		}
	...
	if(write) 
	{
		if(ysplits.size()>0)
		{  
			if(ysplits[splitpos-1]!=-99)
				Y.push_back(ysplits[splitpos-1]);
			else
				Y.push_back(label);
		}
		else
...
{
	int msz,i;
	splits.resize(0); ysplits.resize(0);
	...
	case 2:
		int ft=split_file_load(filename);
		if(ft==0) 	 
			{msz=libsvm_load_data(filename); break;} 
		else
			{msz=binary_load_data(filename); break;}
...
double kernel(int i, int j, void *kparam)
{
  ...
  if (use_ext_K && (i == X.size() || j == X.size())){
...
double calc_multi_class_accuracy(vector< vector <double> > *muYest){
    ...
    acc = 0;
    for (int j = 0; j != data_map[TEST].size(); ++j){
...
double test_current_model(char *fname){
    ...
    Yest.resize(0);
    D.resize(D.size()+1);
    for(i=0;i!=data_map[TEST].size();++i){
 	y=b0; balconstr = 0.0;
...
double test_current_multi_class_model(char *fname){
	...
	for (int i = 0; i != data_map[TEST].size(); ++i){
...
void setup_standard_svm_problem(SVQP2* sv){
    ...
    (*sv).sumflag=use_b0;
    (*sv).epsgr=epsgr;
    for(int j=0;j<data_map[TRAIN].size();++j){
...
void extend_to_universum_problem(SVQP2* sv){
    ...
    for(int j=0;j<data_map[UNIVERSUM].size();++j){
...
void extend_to_tsvm_problem(SVQP2* sv,bool firstloop){
    if (firstloop){
      ...
      for(int j=0;j<data_map[UNLABELED].size();++j){
...
void print_report(vector<double> testerr){
  ...
  fprintf(fp2,"\n# training times\n");
  for (int i = 0; i != training_time.size(); ++i){
  ...
  fprintf(fp2,"\n# accuracies\n");
  for (int i = 0; i != testerr.size()-1; ++i){
...
void search_for_different_labels(){
    ...
    for (int i = 0; i != Y.size(); ++i)
    {
		...
		if (labels.size() == do_multi_class) break;
		found = 0;
    }
...
void setup_folds(vector <  vector <int> >* a, vector <  vector <int> >* b){
  ...
  for (int k = 0; k != labels.size(); ++k){ // for each label
  ...
  vector <int> pointers; for (int k = 0; k != labels.size(); ++k) pointers.push_back(0);
  ...
  if(!balancecv){
	...
	pointers[k] += props[k];
      ...
      for (int k = 0; k != labels.size(); ++k){
	for (int j = pointers[k]; j != label_map[k].size(); ++j){
	  (*a)[i].push_back(label_map[k][j]);
	}
      }
	...
	for(j=0;j<data.size();j++) // add rest of examples to test set
...
void compute_extra_K(){
	...
	for (int i = 0; i != X.size()+1; ++i) global_ext_K[i] = 0.0; // initialize with zero
...
int main(int argc, char **argv)
{
	...
	for (int i = 0; i != data_map[UNIVERSUM].size();++i){
